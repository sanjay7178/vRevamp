...
const get_link = (link_href, reg_no) => {
  const url = link_href.substring(
    link_href.indexOf("'") + 1,
    link_href.lastIndexOf("'")
  );
  const utc = new Date();
  ...
  if (webadress.indexOf("vtop2") === -1) {
    ...
    const params =
      ...
      csrf +
      "&x=" +
      encodeURIComponent(utc.toUTCString());
    return "https://vtop.vitap.ac.in/vtop/" + url.trim() + "?" + params;
    ...
    const params =
      "authorizedID=" + reg_no + "&x=" + encodeURIComponent(utc.toUTCString());
    return "https://vtop.vitap.ac.in/vtop/" + url.trim() + "?" + params;
  }
};
...
const get_link_details = (link_element, index) => {
  ...
  if (link_element.outerText.indexOf("_") === -1) {
    let description =
      link_element.parentNode.parentNode.previousElementSibling.innerText.trim();
    if (description == "") {
      description = "Unnamed";
    }
    const date =
      link_element.parentNode.parentNode.previousElementSibling.previousElementSibling.previousElementSibling.innerText.trim();
...
let course_details = () => {
  ...
  let faculty_slot =
    course_table[10].innerText.trim() + "-" + course_table[11].innerText.trim();
...
const modify_page = () => {
  ...
  document
    .getElementsByClassName("table")[2]
    .insertAdjacentElement("beforebegin", dropdown_file);
  ...
  document
    .getElementsByClassName("table")[2]
    .insertAdjacentElement("beforebegin", dropdown_hover);
  ...
  document
    .getElementsByClassName("table-responsive")[1]
    .appendChild(download_all_u);
  ...
  document
    .getElementsByClassName("table-responsive")[1]
    .appendChild(download_selected_u);
  ...
  document
    .getElementsByClassName("col-md-6 col-md-offset-3")[0]
    .appendChild(download_all_d);
  ...
  document
    .getElementsByClassName("col-md-6 col-md-offset-3")[0]
    .appendChild(download_selected_d);
};
  ...
  if (request.message === "course_page_change") {
    try {
      const loader = setInterval(function () {
        ...
        else reg_no = document.querySelector("#authorizedIDX").value;
        modify_page();
      }, 500);
    } catch (error) {
      // console.log(error);
